...
As such, we model the outcome using a Bayesian multinomial logistic regression. For player i: $P(Y_i = k \mid x_i) = \frac{\exp(\alpha_k + x_i^T \beta_k)}{\sum_{j=1}^K \exp(\alpha_j + x_i^T \beta_j)}$ with Out of League as the baseline class and weakly informative priors on all parameters: $\alpha_k \sim \mathcal{N}(0, 2.5), \quad \beta_k \sim \mathcal{N}(0, 2.5)$
