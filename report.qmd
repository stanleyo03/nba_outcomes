...
The Bayesian multinomial model produces similar substantive conclusions to the multinomial logistic regression, but with a more direct representation of predictive uncertainty. Rather than only assigning one class to a player, the model returns a distribution over possible career outcomes. That is valuable for this project because many highly drafted players are borderline cases early on rather than obvious stars or obvious flops. MCMC diagnostics confirmed convergence: all R-hat values were below 1.05.

The profile comparison below illustrates the key result. A high-producing lottery pick has much more posterior mass on `Starter` and `All-Star`, while a modest late-second-round profile has much more posterior mass on `Out of League` and `Role Player`. The uncertainty intervals around these probabilities are part of the result, not an afterthought, because the entire point of the project is that draft success and failure are uncertain early in a player's career.
...
The main conclusion we found is that early career NBA production contains meaningful information about long-run career outcomes. Across all three models, the most consistent signals are early scoring production, early playing time, and draft position. Draft position alone does not determine success — the EDA shows substantial overlap across outcome tiers — but combined with early performance statistics it meaningfully separates players who wash out from those who become reliable starters or stars. The consistency of this result across a multinomial logistic regression, a Bayesian multinomial model, and a random forest strengthens confidence in the conclusion.
