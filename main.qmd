...
model_data_clean <- model_data |>
  select(outcome, draft_pick, avg_ppg, avg_rpg, avg_apg, avg_spg, avg_bpg,
...
```{r multinom-bootstrap, cache=TRUE}
...
```{r stan-model, cache=TRUE, results="hide", message=FALSE, warning=FALSE}
...
profile_probs <- lapply(1:nrow(profiles_s), function(i) {
  x_i <- profiles_s[i, ]
  log_lik_mat <- cbind(0, sweep(
    matrix(beta_draws %*% x_i, nrow = S, ncol = K - 1),
    2, alpha_draws, "+"
  ))
  probs <- t(apply(log_lik_mat, 1, function(row) exp(row) / sum(exp(row))))
  data.frame(
